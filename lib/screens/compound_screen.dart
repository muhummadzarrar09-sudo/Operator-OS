...
class _CompoundViewState extends ConsumerState<_CompoundView> {
  ...
  void _centerView() {
    ...
    final matrix = Matrix4.identity()
      ...
      ..scale(scale)
      ..translate(-centroid.dx, -centroid.dy);
  ...
  Offset _computeCentroid() {
    double sumX = 0;
    double sumY = 0;
    for (final pos in BuildingConfig.buildingPositions.values) {
      sumX += pos.dx;
      sumY += pos.dy;
    }
    return Offset(sumX / 8, sumY / 8);
  }

  @override
  Widget build(BuildContext context) {
    final daysSinceInstall = DateTime.now().difference(widget.installDate).inDays;
